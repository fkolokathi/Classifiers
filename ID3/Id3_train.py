...
def train():
    ...
    data.remove(
        attributes_names)  # afairesh ths prwths grammhs pou periexei ta onomata twn attiributes mias kai den tha
    # xreiastoun kata to training
    # Run ID3
...
def test():
    o = open("test_result.csv", 'w', newline="\r\n")
    data = []
    tree = train()
    ...
    for entry in data:
        ...
        lis = entry
        lis.pop()
        # the last element of list entry is " " because of the last comma that every
        #  tuple has (sunny,cool,high,TRUE,) in weather.csv file
        lis.append(result)
        length = len(lis) - 1
        i = 0
        for item in lis:
