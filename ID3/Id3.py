...
#ID3 algorithm
#data is a list with sublists each of which contain a turple,attributes is a list with the names of the attributes,goal is the name of the target attribute
def id3(data,attributes,goal):
    ...
    if (len(data)==0) or (len(attributes)-1)<=0:#attributes may contain only the goal item
        return default
    ...
    #    return vals[0]
        ...
        for v in get_attribute_values(data,attributes,best):
            ...
            tree[best][v] = subtree#Me to tree[best] pairnoume to eswteriko {} dhladh ena dictionary.Gia na prosthesoume ena stoixeio v se ayto kanoume tree[best][v] 
            #Me tis sunexeis anadromes tha ftiaxtei arxika ena dictionary pou mesa tha exei eswteriko dictionary sto opoio ta keys tha einai ta padia tou root ktl.
